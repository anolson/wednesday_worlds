class TwitterRideNotifier
  ...
  def ride_tweet(ride)
    %(#{format_date(ride.routes.this_week.occurrence.next_date)}. #{ride.location}. #{format_time(ride.routes.this_week.occurrence.begins_at)}. #{ride.routes.this_week.name} #fb.) 
  end
end
